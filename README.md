...
|               이름               | 역할 분담 |
|:------------------------------:| :--- |
|    ![](image/짱구.png)**하종수**    | 프로젝트 총괄 및 데이터 수집 및 전처리 |
| ![](image/맹구.jpeg)<br/>**김성민** | 프론트엔드 개발 (Streamlit), 데이터 수집 및 전처리 |
|   ![](image/액션가면.png)**송유나**   | 데이터 전처리 및 백엔드 개발 (RAG) |
| ![](image/부리부리대마왕.jpeg)**이나경** | 데이터 전처리 및 백엔드 개발 (RAG) |
|   ![](image/흰둥이.png)**이승혁**    | 백엔드 개발(RAG) 프롬프팅 |
...
*   **이나경:** 초반 프로젝트를 시작할 땐 걱정이 많이 되었지만 든든한 우리 팀원들 덕분에 잘 이겨낼 수 있었습니다. 개인적으로 가장 어려워했던 강의 주간인 만큼, 팀원들에게 누가 되지 않게 제 몫을 해내려 열심히 노력했습니다. 특히 주로 맡았던 챗봇을 구현해보며 RAG 개념에 대해 더 잘 이해하게 된 것 같아 뿌듯했습니다. 이 모든 여정은 저희 팀원들이 없었다면 불가능했을 거라 생각합니다. 감사합니다!!!
